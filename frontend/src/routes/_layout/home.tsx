...
import type { RecipeTrending } from "~/client";
import {
  getRecipesOptions,
...
function Home() {
  const { data, isLoading } = useSuspenseQuery(getRecipesOptions());
...
function LoadingPlaceholder() {
  return (
    <div className="flex items-center justify-center p-8">
      <Loader2Icon className="text-primary animate-spin h-8 w-8 dark:text-primary-light" />
    </div>
  );
}
